...
NGAInputProcessor::NGAInputProcessor()
{
	...
	FNGAGraphUtilityModule* NGAGraphUtilityModule = &FModuleManager::LoadModuleChecked<FNGAGraphUtilityModule>("NGAGraphUtility");
	MyPinFactory = NGAGraphUtilityModule->MyPinFactory;
...
FNGAEventContex NGAInputProcessor::InitEventContex(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent)
{
	...
	for (int i = widgetsUnderCursor.Widgets.Num() - 1; i >= 0; i--)
	{
		...
		if (widgetName == "SGraphPanel")
		{
			...
			else if (graphClassName == "PCGEditorGraph")
			{
				ctx.GraphType = EGraphObjType::EGT_PCG;
			}
		}
		...
		if (widgetName.Contains("GraphPin") || widgetName.Contains("BehaviorTreePin") || widgetName.Contains("OutputPin") || widgetName.Contains("SEnvironmentQueryPin") || widgetName.Contains("SPCGEditorGraphNodePin"))
		{
...
FNGAEventReply NGAInputProcessor::TryProcessAsDupliWireEvent(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent, const FNGAEventContex& Ctx)
{
	...
	if (Ctx.IsCursorInsidePanel && !SlateApp.IsDragDropping())
	{
		if (MouseEvent.IsShiftDown() && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
		{
			...
			{
				TargetPin = Ctx.GraphPin->GetPinObj();
			}
			//todo, support input pin
			else if (MyPinFactory->PayLoadData->OutHoveredOutputPins.Num() > 0 && Ctx.GraphPanel.IsValid())
			{
				TargetPin = MyPinFactory->PayLoadData->OutHoveredOutputPins[0];
			}
			if (TargetPin)
			{
